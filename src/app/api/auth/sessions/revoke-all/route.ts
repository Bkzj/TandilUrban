...
export async function POST(request: Request) {
  return runRouteHandler(request, 'auth.sessions.revoke_all.failed', async (requestId) => {
    ...
    const result = await revokeUserSessionsBulk({
      ...
      code: payload.code,
    }, { client: prisma, encryptionKey: getServerEnvironment().AUTH_ENCRYPTION_KEY ?? '', includeCurrent: true, requestId });
    ...
    return NextResponse.json({ message: 'Todas las sesiones fueron cerradas.', revoked: result.count });
  });
}
