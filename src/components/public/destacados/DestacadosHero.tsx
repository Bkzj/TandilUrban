...
export function DestacadosHero({ totalItems, showcaseImages }: Props) {
  ...
  return (
    <header
      ref={sectionRef}
      className="relative flex min-h-[75vh] flex-col overflow-hidden border-b border-naranja/20"
    >
              ...
              >
                <Icon
                  className={`mx-auto h-4 w-4 sm:h-5 sm:w-5 ${
                    i % 2 === 0 ? 'text-naranja-light' : 'text-emerald-200'
                  }`}
                  aria-hidden
                />
      ...
      >
        <div className="mx-auto flex max-w-7xl flex-wrap items-center justify-center gap-x-8 gap-y-2 px-4 py-3 text-center sm:px-6">
          <p className="text-xs font-medium text-naranja-light/90 sm:text-sm">
            <span className="font-bold text-white">Curadas por Propea Group</span> — calidad y
            engagement
          </p>
      ...
      <motion.a
        ...
        aria-label="Ir a la selección destacada"
      >
